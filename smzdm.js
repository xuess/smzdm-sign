	...
	new Promise(function(resolve, reject) {
		options.callback = function(data, _setCookie) {
			...
			try {
				...
				console.log(error);
				//发邮件
				mailSend('什么值得买获取文章列表报错', new Date().Format("yyyy-MM-dd hh:mm:ss") + ' --- 错误内容：' + error);
			} finally {}
		}
		...
		options.callback = (data, _setCookie) => {
			try {
				...
				if(data.indexOf('"error_code":0') != -1) {
					...
					//发邮件
					mailSend('什么值得买发送评论报错', new Date().Format("yyyy-MM-dd hh:mm:ss") + ' --- 错误内容：' + data);
				}

			} catch(error) {
				console.log(error);
				//发邮件
				mailSend('什么值得买发送评论报错', new Date().Format("yyyy-MM-dd hh:mm:ss") + ' --- 错误内容：' + error);
			} finally {}
		...
		options.callback = (data, _setCookie) => {
			try {
				...
				if(data.indexOf('"error_code":0') != -1) {
					...
					//发邮件
					mailSend('什么值得买签到报错', new Date().Format("yyyy-MM-dd hh:mm:ss") + ' --- 错误内容：' + data);
				}
			} catch(error) {
				console.log(error);
				//发邮件
				mailSend('什么值得买签到报错', new Date().Format("yyyy-MM-dd hh:mm:ss") + ' --- 错误内容：' + error);
			} finally {}
		}
		request(options, cookie, referer);
	});
